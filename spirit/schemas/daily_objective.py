...
class DailyObjectiveSchema(BaseModel):
    primary_objective: str = Field(
        ...,
        min_length=5,
        description="One concrete action for today."
    )

    micro_steps: List[str] = Field(
        default_factory=list,
        max_items=3,
        description="Up to three small steps."
    )
    ...
    difficulty: int = Field(
        ...
        ge=1,
        le=10,
        description="Perceived difficulty from 1–10."
    )

    adjustment_reason: Optional[str] = Field(
        None,
        description="Why this objective was scaled up/down."
    )
